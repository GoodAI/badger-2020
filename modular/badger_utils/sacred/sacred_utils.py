...
from badger_utils.sacred.sacred_reader import SacredReader
from badger_utils.sacred.sacred_writer import SacredWriter
...
class SacredUtils:
    ...
    @property
    def observer(self) -> MongoObserver:
    ...
    @staticmethod
    def add_config_to_experiment(experiment: Experiment, config: Type, seed: Optional[int] = None):
        """
        Register dataclass as config to Sacred Experiment.
        Dataclass must have a `seed` field. A default value must be provided to all additional fields.

        Args:
            experiment: Sacred experiment to be updated
            config: Dataclass to be added to as sacred config.
            seed: Optional fixed seed. When none, random seed is generated
        """
        if seed is None:
            seed = random.randint(0, 2 ** 32)
        experiment.add_config(dataclasses.asdict(config(seed=seed)))

...
class TensorViewer:
    ...
    @rollout_step.setter
    def rollout_step(self, step: int):
        self._rollout_step = step
        self.update()
        if self.rollout_on_change is not None:
            self.rollout_on_change(self._rollout_step)

    def __init__(self, tensor_data: TensorData, plot_count: int = 3, displayed_tensors: Optional[List[str]] = None,
                 rollout_on_change: Optional[Callable[[int], None]] = None, show_slider: bool = True):
        self.tensors = tensor_data.tensor_names
        self.tensor_data = tensor_data
        self._rollout_step = self.n_rollouts - 1
        self.show_slider = show_slider

        tensor_ids = []
        if displayed_tensors is not None:
            for id in displayed_tensors:
                try:
                    tensor_ids.append(tensor_data.tensor_name_to_id(id))
                except KeyError:
                    pass  # Just ignore non existing tensors
    ...
    def _tensor(self, tensor_id: int) -> torch.Tensor:
        return self.tensor_data.tensor(self._rollout_step, tensor_id)
    ...
    def create_layout(self) -> Model:
        use_slider = self.n_rollouts > 1 and self.show_slider

...
class SacredRunsConfigAnalyzer(BokehComponent):
    ...
    def __init__(self, sacred_config: SacredConfig, on_run_selected: Optional[Callable[[int], None]] = None,
                 min_id: int = 2258, max_id: int = 2325, table_width: int = 500):
    ...
    def create_layout(self):
        ...
        self.widget_min_id = TextInput(title='Min Id', value=str(self._min_id), width=100)
        self.widget_min_id.on_change('value', lambda a, o, n: self._update_by_input())
        self.widget_max_id = TextInput(title='Max Id', value=str(self._max_id), width=100)
        self.widget_max_id.on_change('value', lambda a, o, n: self._update_by_input())
        self.widget_config_common = Div(text='')
        self.widget_dt_common = DataTable(source=self.ds_common, width=self._table_width)
